...
func NewHandler(p *Proxy, cc *ClusterConfig, conn net.Conn, forwarder proto.Forwarder) (h *Handler) {
	...
	case types.CacheTypeRedis:
		...
		h.pc = redis.NewProxyConnV2(h.conn, cc.RedisCluster.Auth.CaFile, cc.RedisCluster.Auth.CertFile, cc.RedisCluster.Auth.Password, cc.RedisCluster.Auth.UseTLS)
	case types.CacheTypeRedisCluster:
		//h.pc = rclstr.NewProxyConn(h.conn, forwarder, h.cc.Password, cc)
		h.pc = rclstr.NewProxyConnV2(h.conn, forwarder, cc.RedisCluster.Auth.CaFile, cc.RedisCluster.Auth.CertFile, cc.RedisCluster.Auth.Password, cc.RedisCluster.Auth.UseTLS)
